...
private:
    ...
    RecordId * rids;
    PageId nextPage;
    RC getMinKey( PageFile &pf, KeyType &minKey);
    RC getMaxKey( PageFile &pf, KeyType &maxKey);
public:
    KeyType * keys;
    //key count
    int n;
...
public:
    ...
    //forth 4 bytes store minKey if this is a nonleaf
    //fifth 4 bytes store maxKey if this is a nonleaf
    static const int KEYS_PER_LEAF_PAGE=(PageFile::PAGE_SIZE-sizeof(bool)-sizeof(int)-sizeof(PageId)-2*sizeof(KeyType))/(sizeof(KeyType)+sizeof(RecordId));  
    static const int RECORDIDS_PER_LEAF_PAGE = KEYS_PER_LEAF_PAGE;
    
    static const int KEYS_PER_NONLEAF_PAGE=(PageFile::PAGE_SIZE-sizeof(bool)-sizeof(int)-sizeof(PageId)-2*sizeof(KeyType))/(sizeof(KeyType)+sizeof(PageId))-1;  
