...
BTNode::BTNode()
{
    ...
    keys = (KeyType *)(buffer + sizeof(bool) + sizeof(int) +sizeof(int)+2*sizeof(KeyType));
...
{  
    ...
    memcpy(&minKey, buffer+sizeof(bool)+sizeof(int)+sizeof(PageId), sizeof(KeyType));
    memcpy(&maxKey, buffer+sizeof(bool)+sizeof(int)+sizeof(PageId)+sizeof(KeyType), sizeof(KeyType));
    return 0; 
}
/*
...
{ 
    ...
    memcpy(buffer+sizeof(bool)+sizeof(int)+sizeof(PageId), &minKey, sizeof(KeyType));
    memcpy(buffer+sizeof(bool)+sizeof(int)+sizeof(PageId)+sizeof(KeyType), &maxKey, sizeof(KeyType));
...
{ 
    ...
    if(isLeaf){
        ...
        if(DebugIsEnabled('i')) node.print();
...
void BTNode::print()
{
    int i;
    if(isLeaf){
        printf("pid:%d n:%d Max_n:%d t:%d nextPage:%d minKey:%d maxKey:%d\n", pid, n, KEYS_PER_LEAF_PAGE, getT(), nextPage, minKey, maxKey);
        for(i=0; i<n; i++){
            printf("position:%d\t\tkey:%d\t\trid:{%d,%d}\n",i, keys[i], rids[i].pid, rids[i].sid);
        }
    }else{
        printf("pid:%d n:%d Max_n:%d t:%d minKey:%d maxKey:%d\n", pid, n, KEYS_PER_NONLEAF_PAGE, getT(), minKey, maxKey);
        for(i=0; i<n; i++){
